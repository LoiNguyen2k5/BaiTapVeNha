...
public class UserDaoImpl implements UserDao {
	...
	public User findByEmail(String email) {
		String sql = "SELECT * FROM Users WHERE email = ?";
		try (Connection conn = new DBConnection().getConnection(); // Thay mật khẩu nếu cần
				PreparedStatement ps = conn.prepareStatement(sql)) {

			ps.setString(1, email);
			try (ResultSet rs = ps.executeQuery()) {
				if (rs.next()) {
					// Tái sử dụng code từ phương thức get() để tạo đối tượng User
					User user = new User(sql, sql, sql, sql, sql, 0, sql, null);
					user.setId(rs.getInt("id"));
					user.setUserName(rs.getString("username"));
					...
					return user; // Tìm thấy -> Trả về user
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null; // Không tìm thấy hoặc có lỗi -> Trả về null
	}

	@Override
	public void updateResetToken(String email, String token) {
		String sql = "UPDATE Users SET resetPasswordToken = ? WHERE email = ?";
		try (Connection conn = new DBConnection().getConnection(); // Thay mật khẩu nếu cần
				PreparedStatement ps = conn.prepareStatement(sql)) {
	...
	public User findByResetToken(String token) {
		String sql = "SELECT * FROM Users WHERE resetPasswordToken = ?";
		try (Connection conn = new DBConnection().getConnection(); // Thay mật khẩu nếu cần
				PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				if (rs.next()) {
					// Tái sử dụng code để tạo đối tượng User
					User user = new User(sql, sql, sql, sql, sql, 0, sql, null);
					user.setId(rs.getInt("id"));
					user.setUserName(rs.getString("username"));
					// ... set các trường khác ...
	...
	public void updatePassword(String username, String newPassword) {
		// Xóa token đi sau khi cập nhật mật khẩu
		String sql = "UPDATE Users SET password = ?, resetPasswordToken = NULL WHERE username = ?";
		try (Connection conn = new DBConnection().getConnection(); // Thay mật khẩu nếu cần
				PreparedStatement ps = conn.prepareStatement(sql)) {
